...
A separate Case B stability lemma establishes the exact third-derivative
margin on common inverse charts, with the original frequency labels retained.
...
\end{equation}
Set $P=hX$ and $\delta=h/L$. Since $F\delta=hX=P$, the exact
normalization is
\[
 f_1(Mu,Lv)=P\Gamma_\delta(u,v),
\]
where $\Gamma_\delta$ is the family in the following lemma. The fixed
positive rectangles there are chosen to contain all normalized stationary
domains, with room for the small translations.

\begin{lemma}[Uniform Legendre stability for the Case B phase]
\label{lem:caseB-Legendre-stability}
Let
\[
 \mathcal K^-\Subset\mathcal K\Subset\mathcal K^+
 \Subset(0,\infty)^2
\]
be fixed compact positive rectangles. For sufficiently small $\delta>0$,
define
\begin{equation}\label{eq:caseB-stability-exact}
 \Gamma_\delta(u,v)
 =\frac{2\sqrt u}{3\sqrt3}
   \frac{(v+\delta)^{-1/2}-v^{-1/2}}{\delta},
 \qquad
 \Gamma_0(u,v)=-\frac1{3\sqrt3}u^{1/2}v^{-3/2}.
\end{equation}
There are constants $\delta_*>0$, $C>0$ and $c>0$, independent of
$\delta$, such that for $0\le\delta\le\delta_*$,
\begin{equation}\label{eq:caseB-stability-C5}
 \norm{\Gamma_\delta-\Gamma_0}_{C^5(\mathcal K)}\le C\delta.
\end{equation}
Also $\norm{\Gamma_\delta}_{C^m(\mathcal K)}\le C_m$ for every fixed
$m$, and the determinant and first-coordinate pivot have uniform margins:
\[
 \det\Hess\Gamma_\delta\le-c<0,\qquad
 \Gamma_{\delta,uu}\ge c>0
 \quad\hbox{on }\mathcal K.
\]

There is a fixed finite cover of $\mathcal K^-$ by source charts $C_j$
contained in buffered balls $B_j\Subset\operatorname{int}\mathcal K$,
and there are common mixed and gradient inverse rectangles
$A_j^-\Subset A_j^+$ and $Q_j^-\Subset Q_j^+$ for all such $\delta$,
including $0$. They satisfy the compatibility inclusions of
Lemma~\ref{lem:buffered-inverses}. In particular
\[
 \nabla\Gamma_\delta(C_j)\subset Q_j^-,\qquad
 I_{\delta,j}=(\nabla\Gamma_\delta|_{B_j})^{-1}:Q_j^+\longrightarrow B_j
\]
is defined on a neighborhood of $\overline{Q_j^+}$. All chart counts,
side lengths and normalized buffers are independent of $\delta$.

Retain the frequency labels
\begin{equation}\label{eq:caseB-stability-labels}
 (r,s)=(\Gamma_{\delta,u},\Gamma_{\delta,v}),
 \qquad y=(r,s),
\end{equation}
and on each specified branch put
\[
 \Lambda_{\delta,j}(y)
 =\Gamma_\delta(I_{\delta,j}(y))-y\cdot I_{\delta,j}(y).
\]
Then
\begin{equation}\label{eq:caseB-stability-C3}
 \norm{\Lambda_{\delta,j}-\Lambda_{0,j}}_{C^3(Q_j^+)}\le C\delta,
\end{equation}
and throughout the common outer rectangle,
\begin{equation}\label{eq:caseB-stability-third}
 (\Lambda_{\delta,j})_{sss}(r,s)\le-c<0.
\end{equation}
Here $s$ is the \emph{second} frequency coordinate, dual to $v$.

The phase graphs, their derivatives of every fixed order, the specified
mixed and gradient inverse graphs, and the chart restrictions form
semialgebraic families of uniformly bounded format. Starting from any
sharp-domain family of fixed format, its A correlations and the actual
transformed domains in Lemma~\ref{lem:sharpB}, including arbitrary
rectangular cuts, have uniformly bounded format. Every coordinate fiber
therefore has a uniformly bounded number of intervals and points.
These assertions are joint in $\delta$ and all sharp parameters,
including the value $\delta=0$.
\end{lemma}
\begin{proof}
Choose $\delta_*>0$ initially so that
\[
 (u,v)\in\mathcal K,\quad 0\le\tau\le1,\quad
 0\le\delta\le\delta_*
 \quad\Longrightarrow\quad
 (u,v+\tau\delta)\in\mathcal K^+.
\]
The fundamental theorem of calculus gives
\begin{equation}\label{eq:caseB-stability-integral}
 \Gamma_\delta(u,v)=\int_0^1\Gamma_0(u,v+\tau\delta)\dd\tau.
\end{equation}
For every spatial multi-index $|\alpha|\le5$,
\[
 \partial^\alpha(\Gamma_\delta-\Gamma_0)(u,v)
 =\delta\int_0^1\int_0^\tau
   \partial_v\partial^\alpha\Gamma_0(u,v+q\delta)
   \dd q\dd\tau.
\]
The derivatives on the right are bounded on the fixed positive rectangle
$\mathcal K^+$. This proves \eqref{eq:caseB-stability-C5}.
Differentiating \eqref{eq:caseB-stability-integral} to any fixed order
also proves the uniform upper bounds, with no negative power of $\delta$.

Direct differentiation of the model gives
\begin{equation}\label{eq:caseB-stability-Hessian}
 \Gamma_{0,uu}=\frac1{12\sqrt3}u^{-3/2}v^{-3/2}>0,
 \qquad \det\Hess\Gamma_0=-\frac1{18uv^5}<0.
\end{equation}
Both margins are uniform on $\mathcal K$. The $C^2$ consequence of
\eqref{eq:caseB-stability-C5}, together with the uniform Hessian upper
bound, transfers them to $\Gamma_\delta$ after decreasing $\delta_*$.
Thus the first coordinate is a uniformly valid pivot.

Apply Lemma~\ref{lem:buffered-inverses} with $F_0=\Gamma_0$ and
$F=\Gamma_\delta$, using $\mathcal K^-$ as the compact source set and
$\operatorname{int}\mathcal K$ as its ambient neighborhood. The common
rectangles for $\nabla\Gamma_\delta$ and for
$M_\delta(u,v)=(\Gamma_{\delta,u}(u,v),v)$ have the stated compatibility.
Since the model is fixed, both the source centers and target rectangles
can be fixed independently of $\delta$. Each inverse graph is restricted
to its specified source ball. The model gradient image lies in
$\{r<0<s\}$, so the common gradient rectangles lie in that quadrant.

We give the comparison on one such fixed target rectangle in detail.
Suppress $j$, put $H_\delta=\nabla\Gamma_\delta$, and write
$I_\delta=H_\delta^{-1}$. The uniform lower Lipschitz bound
\eqref{eq:inverse-co-lipschitz} on the source ball gives
\begin{align*}
 |I_\delta(y)-I_0(y)|
 &\le C|H_\delta(I_\delta(y))-H_\delta(I_0(y))|\\
 &=C|H_0(I_0(y))-H_\delta(I_0(y))|\le C\delta.
\end{align*}
Let $A_\delta(y)=\Hess\Gamma_\delta(I_\delta(y))$. Implicit
differentiation gives
\[
 DI_\delta(y)=A_\delta(y)^{-1}
\]
and, for vectors $a,b\in\R^2$,
\[
 D^2I_\delta(y)[a,b]
 =-A_\delta(y)^{-1}D^2H_\delta(I_\delta(y))
                     [DI_\delta(y)a,DI_\delta(y)b].
\]
The inverse matrices are uniformly bounded. Comparing the first formula
for $\delta$ and $0$, using
$A^{-1}-B^{-1}=A^{-1}(B-A)B^{-1}$, gives a $C^1$ inverse difference
of size $O(\delta)$. Comparing the second formula then gives
\[
 \norm{I_\delta-I_0}_{C^2(Q_j^+)}\le C\delta.
\]
Indeed the differences of derivatives of $\Gamma_\delta$ and
$\Gamma_0$ are $O(\delta)$ by \eqref{eq:caseB-stability-C5}, and
evaluating a model derivative at $I_\delta(y)$ instead of $I_0(y)$
costs $O(\delta)$ by its bounded next derivative.

The defining Legendre formula first yields
$\norm{\Lambda_\delta-\Lambda_0}_{C^0(Q_j^+)}\le C\delta$.
Since $\nabla\Lambda_\delta=-I_\delta$, the $C^2$ inverse comparison
gives \eqref{eq:caseB-stability-C3}. In particular all comparisons
are on the same target rectangle and between the specified branches.

The model gradient equations, with the labels
\eqref{eq:caseB-stability-labels}, are
\[
 r=-\frac1{6\sqrt3}u^{-1/2}v^{-3/2},\qquad
 s=\frac1{2\sqrt3}u^{1/2}v^{-5/2}.
\]
Their unique positive solution is
\[
 v=(36(-r)s)^{-1/4},\qquad u=\frac{s}{3(-r)}v,
 \qquad r<0<s.
\]
Consequently
\begin{equation}\label{eq:caseB-stability-model-Legendre}
 \Lambda_0(r,s)=-\frac{2\sqrt6}{9}(-r)^{-1/4}s^{3/4}.
\end{equation}
Three differentiations in the second frequency coordinate give
\begin{equation}\label{eq:caseB-stability-model-third}
 (\Lambda_0)_{sss}(r,s)
 =-\frac{5\sqrt6}{96}(-r)^{-1/4}s^{-9/4}.
\end{equation}
Equivalently, in source coordinates,
\begin{equation}\label{eq:caseB-stability-third-pullback}
 (\Lambda_0)_{sss}(\nabla\Gamma_0(u,v))
 =-\frac{15}{4}\frac{v^6}{u}.
\end{equation}
Let
\[
 \overline u=\max_{(u,v)\in\mathcal K}u,
 \qquad \underline v=\min_{(u,v)\in\mathcal K}v>0,
 \qquad m_* =\frac{15\underline v^6}{4\overline u}>0.
\]
Every model inverse value on $Q_j^+$ belongs to $B_j\subset\mathcal K$.
Thus $(\Lambda_{0,j})_{sss}\le-m_*$ throughout $Q_j^+$.
Decrease $\delta_*$ so that the constant in
\eqref{eq:caseB-stability-C3} satisfies $C\delta_*\le m_*/2$.
Then $(\Lambda_{\delta,j})_{sss}\le-m_*/2$. Taking the smaller of
this margin and the Hessian margins proves all assertions involving $c$.

For complexity, rationalize the exact difference quotient:
\[
 \Gamma_\delta(u,v)
 =-\frac{2}{3\sqrt3}
   \frac{\sqrt u}{\sqrt v\sqrt{v+\delta}
                      (\sqrt v+\sqrt{v+\delta})}.
\]
The equality $w=\Gamma_\delta(u,v)$ is therefore equivalent to the
existence of $a,b,d>0$ satisfying
\begin{equation}\label{eq:caseB-stability-graph}
 a^2=u,\qquad b^2=v,\qquad d^2=v+\delta,\qquad
 3\sqrt3\,bd(b+d)w=-2a.
\end{equation}
These are fixed-degree polynomial conditions. Positivity selects the
radical branches uniquely, and the same formula at $\delta=0$ gives
$\Gamma_0$. Thus this single graph description includes the limiting
parameter without division by $\delta$.

Lemma~\ref{lem:sa-calculus} gives bounded-format graphs for derivatives
of every fixed order. Alternatively, repeated differentiation of the
rationalized expression, using $\partial_u a=1/(2a)$,
$\partial_v b=1/(2b)$ and $\partial_v d=1/(2d)$, produces rational
expressions of bounded degrees in the positive auxiliary variables.
The selected gradient inverse graph is the relation
\[
 (u,v)\in B_j,\qquad
 r=\Gamma_{\delta,u}(u,v),\qquad s=\Gamma_{\delta,v}(u,v).
\]
Uniqueness on $B_j$ was proved above. The mixed inverse graph retains
$v$ in place of the second gradient coordinate. Adjoining
$z=\Gamma_\delta(u,v)-ru-sv$ gives the Legendre graph. The chart
rectangles and balls are fixed sets of bounded format. All these
constructions therefore use fixed-format graph relations and projections;
the integral identity is used only for analytic norm bounds.

An A correlation has domain $D\cap(D-\mathbf h)$ with the shift as a free
parameter. The actual B domains use exactly the fiber interiors,
specified map images and rectangular cuts in
\eqref{eq:sharp-domain-family}, including every source and branch
restriction. Lemmas~\ref{lem:sa-calculus} and~\ref{lem:sharp-fibers}
give a common format bound with all cut endpoints and $\delta$ retained
as free real parameters. Positive physical scalings preserve this bound.
Specializing these parameters and one coordinate leaves a univariate
description with uniformly bounded polynomial counts and degrees.
Its roots give a uniform bound on the number of interval and point
components. This also treats coincident endpoints, empty fibers and
$\delta=0$. Integrality is imposed only when summing over the resulting
real domains. This proves the remaining claims.
\end{proof}

For every nonempty correlation, \eqref{eq:caseB-cap} gives
\[
 0<\delta=\frac hL<\frac qL\le\frac1{C\log x}\le\delta_*
\]
for sufficiently large $x$, uniformly over the box and the shift.
Lemma~\ref{lem:caseB-Legendre-stability} therefore applies to the exact
phase. Its $C^{20}$ bound, determinant margin and first-coordinate pivot
allow the fixed-order remainder $\Rfine(M,L,P)$ in
Lemma~\ref{lem:sharpB}. Use the specified source charts of the stability
lemma; every actual transformed domain, including both Abel cuts, then
lies in its common inner gradient rectangle $Q_j^-$.

Suppose first that both dual lengths
\[
 R_1=P/M,\qquad R_2=P/L
\]
are at least one. In physical dual coordinates $(\xi,\zeta)$ the exact
phase is $P\Lambda_{\delta,j}(\xi/R_1,\zeta/R_2)$, and
\begin{equation}\label{eq:caseB-physical-third}
 \partial_\zeta^3\left[P\Lambda_{\delta,j}
               (\xi/R_1,\zeta/R_2)\right]
 =\frac P{R_2^3}(\Lambda_{\delta,j})_{sss}
               (\xi/R_1,\zeta/R_2).
\end{equation}
The absolute value is uniformly comparable to $P/R_2^3$ on the outer
inverse rectangle. The same lemma bounds the number of components in
every actual second-coordinate fiber. Applying \eqref{eq:third-test}
to its intervals, and allowing $O(1)$ isolated points and endpoint
changes, bounds each such fiber by
\[
 O\left(1+R_2^{1/2}P^{1/6}+R_2P^{-1/6}\right).
\]
There are $O(R_1)$ first-coordinate frequencies. Multiplication by the
two-dimensional B factor $ML/P$ therefore bounds the dual main term by
\begin{align*}
 &\frac{ML}{P}R_1
       \left(1+R_2^{1/2}P^{1/6}+R_2P^{-1/6}\right)\\
 &\hspace{25mm}=L+\sqrt L\,P^{2/3}+P^{5/6},
\end{align*}
up to a uniform constant. The $L$ term is already in $\Rfine(M,L,P)$.
Including the complete finer remainder yields
